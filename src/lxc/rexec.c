...
#include <string.h>
#include <unistd.h>
#include <sys/mman.h>  // 1. 新增：补充shm_open/shm_unlink依赖头文件
#include <sys/stat.h>
#include <fcntl.h>
#include <sys/types.h>
...
static int memfd_create_compat(const char *name, unsigned int flags) {
    (void)flags;
    char shm_name[64];
    int ret = snprintf(shm_name, sizeof(shm_name), "/%s_%d", 
                      name ?: "lxc_memfd", getpid());
...
static void lxc_rexec_as_memfd(char **argv, char **envp, const char *memfd_name)
{
	...
	// 3. 修改：统一fd_size声明，低版本用fstat获取大小
#if IS_BIONIC
	off_t fd_size = -1;  // 统一声明，替代原分支内重复定义
#else
	struct stat st = {0};
#endif
